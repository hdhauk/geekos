...
void sigusr1_handler(int signal) {
    Print("In my USR1 handler\n");
    Kill(Get_PID(), SIGUSR2);
}


void kill_handler(int signal) {
    Print("In my kill handler\n");
    Exit(0);
}

void sigusr2_handler(int signal) {
    Print("In my USR2 handler\n");
...
int main() {
    ...
    if(child_pid > 0) {
        ...
        rc = Signal(sigusr1_handler, SIGUSR1);
        rc = Signal(sigusr2_handler, SIGUSR2);
        assert(rc == 0);
        Print("Sending SIGUSR1 to myself...\n");
        rc = Kill(Get_PID(),SIGUSR1);
        Print("kill = %d\n", rc);



        rc = Signal(SIG_IGN, SIGPIPE);
        assert(rc != 0);
